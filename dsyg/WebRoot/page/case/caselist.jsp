...
<script type="text/javascript">
	...
	function add() {
		document.mainform.action = '<c:url value="/case/showAddCaseAction.action"></c:url>';
	...
	function upd() {
		...
		if(id == "") {
			...
			document.mainform.action = '<c:url value="/case/showUpdCaseAction.action"></c:url>' + "?updCaseId=" +id;
			document.mainform.submit();
		}
	}
	...
	function del() {
		...
		if(id == "") {
			...
			if(confirm("确定删除该记录吗？")) {
				document.mainform.action = '<c:url value="/case/delCaseAction.action"></c:url>' + "?delCaseId=" + id;
				document.mainform.submit();
			}
		}
	}
	...
	function changePage(pageNum) {
		...
		document.mainform.action = '<c:url value="/case/turnCaseAction.action"></c:url>';
		document.mainform.submit();
	}
...
<div id="main" style="margin-top:2px;">
	<div  class="header_bg">
		<div class="content">
			<div class="user_bg">
				<div class="user">
					<div class="user_tittle">后台管理-----案例介绍一览
					</div>
				</div>
			</div>
			...
			<div class="product_search main_body">
				<s:form id="mainform" name="mainform" method="POST">
					...
					<h3 class="s_select">产品类型
						<select name="queryCaseDto.fieldcode" id="fieldcode">
							<option value="" selected="selected">请选择</option>
							<s:iterator value="goodsList" id="goodsList" status="st1">
								<s:if test="%{goodsList[#st1.index].code == queryCaseDto.fieldcode}">
			  		...
			  		<div class="search1" style="margin-top: 10px;">
						<h3>　案例标题
							<input class="keyword" name="queryCaseDto.title" id="queryTitle" value="<s:property value="queryCaseDto.title"/>" type="text" />
					...
					<table class="product_tab" width="100%" border="1" cellspacing="5" cellpadding="10">
						...
						<s:iterator id="caseList" value="caseList" status="st1">
							...
							<tr class="bg2">
								...
								<td>
									<s:iterator value="goodsList" id="goodsList" status="st2">
										<s:if test="%{goodsList[#st2.index].code == caseList[#st1.index].fieldcode}">
