...
<title>东升盈港-库存输入</title>
<script type="text/javascript">
	function add() {
		if(checkData()) {
			if(confirm("确定添加吗？")) {
				document.mainform.action = '<c:url value="/warehouse/addWarehouseAction.action"></c:url>';
...
<div id="main" style="margin-top:2px;">
	<div class="header_bg">
		<div class="content warehouse_input">
			<div class="user_bg">
				<div class="user">
					<div class="user_tittle">后台管理------库存输入
					</div>
				</div>
			</div>
			<div class="main_body">
				<s:form id="mainform" name="mainform" method="POST" enctype="multipart/form-data">
					...
					<s:hidden name="addWarehouseDto.res03" id="res03"></s:hidden>
					<s:hidden name="addWarehouseDto.res04" id="res04"></s:hidden>
						...
						<tr>
							...
							<td>
								<dl>
									<dt>数量：</dt>
									<dd>
										<s:textfield name="addWarehouseDto.item01" id="item01" cssStyle="width:300px;" maxlength="10" theme="simple"></s:textfield>
									</dd>
								</dl>
								<dl>
									<dt>乱尺：</dt>
									<dd>
										<s:if test='addWarehouseDto.res03 == "1"'>
								...
								<dl>
									<dt>单位：</dt>
									<dd>
										<select name="addWarehouseDto.res01" id="res01">
											<s:iterator value="unitList" id="unitList" status="st2">
												<option value="<s:property value="code"/>" <s:if test="%{unitList[#st2.index].code == addWarehouseDto.res01}">selected</s:if>><s:property value="fieldname"/></option>
											</s:iterator>
										</select>
									</dd>
								</dl>
								<dl>
									<dt>发送天数：</dt>
									<dd>
										<s:textfield name="addWarehouseDto.item03" id="item03" cssStyle="width:300px;" maxlength="10" theme="simple"></s:textfield>
									</dd>
								</dl>
								<dl>
									<dt>期货：</dt>
									<dd>
										<s:if test='addWarehouseDto.res04 == "1"'>
								...
								<dl>
									<dt>产地：</dt>
									<dd>
										<select name="addWarehouseDto.res02" id="res02">
											<s:iterator value="makeareaList" id="makeareaList" status="st2">
												<option value="<s:property value="code"/>" <s:if test="%{makeareaList[#st2.index].code == addWarehouseDto.res02}">selected</s:if>><s:property value="fieldname"/></option>
											</s:iterator>
										</select>
									</dd>
								</dl>
